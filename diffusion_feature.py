...
def preprocess(data, preprocess="diffusion", num_propagations=10, p=None, alpha=None, use_cache=True, post_fix="", is_ogb_submission=False):
    if use_cache:
        try:
...
#         result = lp(adj, data.y.data, num_propagations, p = p, alpha = alpha, preprocess = preprocess)
    if preprocess == "diffusion":
        result = diffusion(data.x.numpy(), adj,
                           num_propagations, p=p, alpha=alpha)

    embedding_dir = 'embeddings'
    if not os.path.exists(embedding_dir):
        os.makedirs(embedding_dir)
    torch.save(result, f'{embedding_dir}/{preprocess}{post_fix}.pt')
